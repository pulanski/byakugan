...
fn main() -> Result<()> {
    ...
    let initial_file_path =
        std::env::args_os().nth(1).ok_or_else(|| eyre!("Usage: ./lazy-input <input-file>"))?;
    ...
    loop {
        ...
        for log in db.logs.write().drain(..) {
            // for log in db.logs.lock().drain(..) {
            eprintln!("{log}");
        }
...
impl Db for Database {
    fn input(&self, path: PathBuf) -> Result<File> {
        let path =
            path.canonicalize().wrap_err_with(|| format!("Failed to read {}", path.display()))?;
        Ok(match self.files.entry(path.clone()) {
            ...
            Entry::Vacant(entry) => {
                ...
                // race conditions.
                let watcher = &mut *self.file_watcher.lock();
                watcher.watcher().watch(&path, RecursiveMode::NonRecursive).unwrap();
                let contents = std::fs::read_to_string(&path)
...
impl salsa::Database for Database {
    fn salsa_event(&self, event: salsa::Event) {
        ...
        if let salsa::EventKind::WillExecute { .. } = event.kind {
            self.logs.write().push(format!("{:?}", event.debug(self)));
        }
    }
}
...
impl Diagnostic {
    fn push_error(db: &dyn Db, file: File, error: Report) {
        Diagnostic::push(
            ...
            format!(
                "Error in file {}: {:?}\n",
                file.path(db).file_name().unwrap_or_else(|| "<unknown>".as_ref()).to_string_lossy(),
                error,
            ),
        )
    }
    ...
    let value = match lines.next().map(|line| (line.parse::<u32>(), line)) {
        ...
        Some((Err(e), line)) => {
            Diagnostic::push_error(
                ...
                input,
                Report::new(e)
                    .wrap_err(format!("First line ({line}) could not be parsed as an integer")),
            );
            0
        }
        None => {
...
fn sum(db: &dyn Db, input: ParsedFile) -> u32 {
    input.value(db) + input.links(db).iter().map(|&file| sum(db, file)).sum::<u32>()
}
